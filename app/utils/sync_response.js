...
const sync_response=async(userId,phone_number,setLoading,formsStats,setStatsData)=>{
    setLoading(true)
                    ...
                    setLoading(true)
                       ...
                       saved[0].saved.map( async single_form_data=>{
                           ...
                           var queryString = Object.keys(single_form_data)
                                .map((key) => {
                                    return (
                                    encodeURIComponent(key) + "=" + encodeURIComponent(single_form_data[key])
                                    );
                                })
                                .join("&");
                               ...
                               const { data } = await axios.get(
                                ...
                                if(data.status){
                                    ...
                                    const onlineData = await AsyncStorage.getItem(`online-${form.formId}`);
                                    let dd = [];
                                    ...
                                    await AsyncStorage.setItem(`online-${form.formId}`, JSON.stringify(dd));

                                    //lets remove from local storage-saved
                                    const savedData = await AsyncStorage.getItem(`saved-${form.formId}`);
                                    let ds = [];
                                    ...
                                    ds = ds.filter(item => {return !(_.isEqual(item,single_form_data))})
                                    //console.log("DS",ds)
                                    await AsyncStorage.setItem(`saved-${form.formId}`, JSON.stringify(ds));

                                    await AsyncStorage.setItem(
                                        "@Stats",
                                        JSON.stringify({
                                        ...formsStats,
                                        [`saved-${form.formId}`]: formsStats?.[`saved-${form.formId}`] - 1,
                                        [`online-${form.formId}`]: formsStats?.[`online-${form.formId}`] + 1
                                        })
                                    );
                                    setStatsData({
                                        ...formsStats,
