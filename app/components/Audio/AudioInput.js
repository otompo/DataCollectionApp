...
export const UserAudioInput = ({
    ...
    async function startRecording() {
      ...
      try {
        ...
        await recording.setOnRecordingStatusUpdate((status)=>{setRecorderStatus(status)})
    ...
    return (
      <View style={{ marginLeft: 24 }}>
        ...
        <View style={{flexDirection:'column',justifyContent:'center',alignItems:'center'}}>
            <Text style={{fontSize:15,fontWeight:'bold'}}>Playing Sound</Text>
            <Text>{convertMsToHM(parseInt(status.positionMillis)) || timer}</Text>
            {status?.isPlaying? <Icon size={35} name="pause" color="yellow" onPress={()=>audio.pauseAsync()} />:
             <Icon size={35} name="play" color="green" onPress={()=>audio.playAsync()} />}
        </View> }
  
        {recorderStatus?.isRecording && <View><Text style={{fontWeight:'bold'}}>{convertMsToHM(parseInt(status.durationMillis)) || timer}</Text>
